...
For `SessionEnded` events (Story 5.3 — daemon-observed liveness), `payload` carries the mechanical observation as a JSON object: `{"reason":"no_pid_at_upgrade"|"pid_dead"|"pid_superseded","pid":<number or null>,"observed_at_ms":<epoch_ms>}`. `reason` records *which observation* ended the session — all three are mechanical facts, not instructions: `pid_dead` (the probe saw `kill(pid, 0)` return `ESRCH`), `no_pid_at_upgrade` (the row never carried a PID), and `pid_superseded` (Story 5.11 / ADR 0009 — the session's PID was observed to roll over to a newer session_id, i.e. a successor session emitted on the same PID; observed event-driven at ingest time, not OS-confirmed dead). Interpretation — hide a superseded session, dim it, or show its final state — is the presenter's (Axiom 4). Treat `reason` as an opaque string: a future daemon may add values, and a presenter only needs to special-case the ones it renders distinctly. Presenters that want to render "session is dead" subscribe to `state.session.*` and watch for `current_state: "Ended"` — they do NOT need to call `kill(pid, 0)` themselves.
...
| `SessionEnded` | yes — daemon-observed | The daemon observed the session end: the 5-second liveness probe saw `last_pid` is no longer a live OS process (`reason: pid_dead` / `no_pid_at_upgrade`, Story 5.3), or an event-driven check saw that PID roll over to a newer session_id while still alive (`reason: pid_superseded`, Story 5.11 / ADR 0009). Projection transitions to `Ended`; the specific `reason` rides the payload. **Non-terminal** — a subsequent hook event (e.g. `claude --resume`) transitions back out via the normal rules |
