...
`SessionCurrentState` is one of `Idle`, `Working`, `WaitingInput`, `Ended`, `Unknown`. `WaitingInput` means the session is blocked on user input with work queued behind the answer (`permission_prompt` / `elicitation_dialog`, incl. `AskUserQuestion`); as of Story 5.6 / ADR 0005 these are the only two `notification_type` values that *transition a session into* it. `idle_prompt` does NOT transition a session into `WaitingInput` — it is a transient idle nudge (~60s after a turn ends) that preserves prior state (reads `Idle` after a normal turn-end). Because it preserves prior, a session already in `WaitingInput` from a pending block stays `WaitingInput` when a later idle nudge arrives — the nudge neither creates nor clears the blocked state. The `Ended` variant (Story 5.3) is daemon-observed: the session's `last_pid` is no longer a live OS process. **`Ended` is non-terminal** — a session can transition out on the next hook event (e.g. a `UserPromptSubmit` from `claude --resume` returns the session to `Working`). The `Unknown` variant is the additive-compat catch-all added in Story 4.4 (via `#[serde(other)]`): a future v1.x daemon may introduce new state values (e.g. `Compacting`, `AwaitingApproval`) which v1.0 presenters MUST decode as `Unknown` rather than erroring on the tag. The daemon never *produces* `Unknown` — it's decode-only, same shape as `ServerMessage::Unknown` (Story 2.1). Source: [`crates/protocol/src/state.rs`](../crates/protocol/src/state.rs).
...
- **`notification_type` extraction** (Story 5.3; `idle_prompt` reclassified in Story 5.6 / ADR 0005). The adapter (NOT the shim) reads the upstream Claude Code `notification_type` field on `Notification`-kind payloads and uses the typed value to drive the projection transition: input-required types (`permission_prompt`, `elicitation_dialog`) → `WaitingInput`; transient types (`idle_prompt`, `auth_success`, `elicitation_response`, `elicitation_complete`) and any unknown/future value → preserve prior `current_state`. `idle_prompt` is the idle nudge Claude fires ~60s after a turn ends, NOT a block, so it preserves prior (reads `Idle` after a normal turn-end) rather than producing `WaitingInput`. The typed value is NOT surfaced on `SessionState` or the wire `StateFrame` — it stays in `events.payload` (verbatim) for presenters that want richer rendering by subscribing to the events stream.
...
| `Notification` | yes | A non-tool side-channel event. Transition to `WaitingInput` depends on the payload's `notification_type` field (Story 5.3; narrowed by Story 5.6 / ADR 0005): `permission_prompt`, `elicitation_dialog` → `WaitingInput`; `idle_prompt`, `auth_success`, `elicitation_response`, `elicitation_complete`, future-unknown, or missing → preserve prior |
