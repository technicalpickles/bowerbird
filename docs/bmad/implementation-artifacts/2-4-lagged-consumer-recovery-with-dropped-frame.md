...
- [x] [Review][Patch] Cursor advances before a matching Event frame is actually delivered [crates/daemon/src/api/ws.rs:291] — Task 3.2 and the Dev Notes say `last_delivered_event_id` tracks successful Event dispatches. The implementation currently sets `last_delivered_event_id = Some(ev.event_id)` in both the main `rx.recv()` path (`crates/daemon/src/api/ws.rs:291-294`) and `drain_backlog_under_state` (`crates/daemon/src/api/ws.rs:545-550`) before calling `dispatch_envelope`. But `dispatch_envelope` first checks `subscriptions.iter().any(|t| t.matches(envelope))` and returns `true` without sending anything when no topic matches (`crates/daemon/src/api/ws.rs:679-681`). That means a state-only client, a narrow `events.<source>.<session_id>` subscriber, or any subscriber seeing unrelated events can have its recovery cursor advanced by Event envelopes it never received. The next Dropped frame will compute `first_dropped_event_id` from that false cursor instead of from the presenter's actual last delivered event, corrupting the recovery signal. Fix direction: make `dispatch_envelope` report whether it actually sent a matching `Event` frame, or compute the match before cursor mutation, and update `last_delivered_event_id` only after a successful matching Event send. Add regression coverage with a nonmatching Event before lag; for a pure `state.*` subscriber, first lag should still use the `EventId(0)` sentinel described in Dev Notes. **Resolved:** `dispatch_envelope` now returns a `DispatchOutcome` enum (`Sent`/`Filtered`/`Closed`) instead of bare `bool`. Both call sites (`rx.recv()` main arm and `drain_backlog_under_state`) advance `last_delivered_event_id` only on `Sent` for an `Event` envelope. Regression test `state_only_subscriber_does_not_advance_cursor_on_unrelated_events` asserts `first_dropped_event_id == EventId(0)` for a `state.session.*` subscriber after a flood of unrelated Events; verified by stash-reverting the fix and observing the test fail with `EventId(6)`.
- [x] [Review][Patch] Time-sensitive coalescing tests use wall-clock sleeps instead of deterministic time [crates/daemon/tests/contract_daemon.rs:5067] — The story's deterministic test guidance for coalescing called for virtual time (`tokio::test(start_paused = true)` plus `tokio::time::advance`) so the policy is not scheduler/load sensitive. The implementation documents a tradeoff and uses wall-clock timing instead: `sustained_lag_does_not_storm_dropped_frames` drives a `std::time::Instant::now()` deadline and real `tokio::time::sleep(Duration::from_millis(5))` / `sleep(Duration::from_millis(2))` loops (`crates/daemon/tests/contract_daemon.rs:5067-5102`), and `coalesce_window_resets_after_silence` waits with a real 300ms sleep (`crates/daemon/tests/contract_daemon.rs:5325-5328`). That leaves the most timing-sensitive part of Story 2.4 dependent on CI scheduler behavior and contradicts the specified deterministic coverage. Fix direction: either convert these tests to paused Tokio time, or factor the coalescing policy into a small helper/sink/clock seam that can be tested deterministically without relying on real TCP socket timing. Keep one end-to-end smoke test if useful, but assert the coalescing window contract in deterministic unit-level coverage. **Resolved:** Factored the coalescing decision out of `emit_dropped_or_coalesce` into a pure synchronous `coalesce_decision(now, last_dropped_at, &mut pending, last_delivered, n, window) -> CoalesceDecision` that takes `tokio::time::Instant` values explicitly. Seven new unit tests in `crates/daemon/src/api/ws.rs::tests` (`coalesce_first_lag_ever_emits`, `coalesce_first_lag_uses_cursor_plus_one`, `coalesce_within_window_suppresses_and_accumulates`, `coalesce_window_expired_emits_pending_plus_n`, `coalesce_boundary_equal_to_window_suppresses`, `coalesce_pure_repeated_calls_within_window_accumulate`, `coalesce_count_one_first_eq_last`) cover the policy deterministically by constructing fake `Instant` values via arithmetic — no scheduler, no TCP, no `tokio::time::pause`. The existing e2e tests stay as smoke coverage of the wire integration; the policy contract is now CI-stable.
- [ ] [Review][Patch] Lag before any active subscription emits an unsolicited Dropped frame [crates/daemon/src/api/ws.rs:124] — The broadcast receiver is subscribed before the WebSocket upgrade completes (`let rx = state.broadcaster.subscribe()`), while the per-connection `subscriptions` set starts empty (`crates/daemon/src/api/ws.rs:187`). Both lag paths emit `DroppedFrame` without checking whether any topic is currently active: the main `RecvError::Lagged(n)` arm calls `emit_dropped_or_coalesce` at `crates/daemon/src/api/ws.rs:305-319`, and the Subscribe/Unsubscribe drain path does the same for `TryRecvError::Lagged(n)` at `crates/daemon/src/api/ws.rs:567-579`. Normal envelopes are filtered by `dispatch_envelope`, but lag bypasses topic matching completely. A client can therefore connect, idle while unrelated traffic exceeds capacity, then subscribe and receive a `dropped` frame for events that happened before it asked for any stream; the same shape can happen after unsubscribing from all topics and then flooding the hub. Fix direction: when `subscriptions.is_empty()`, treat lag as a silent fast-forward rather than a recoverable delivery gap. Apply this guard in both the main `rx.recv()` lag arm and `drain_backlog_under_state` under the old subscription set. Add regression tests for `connect -> idle flood -> subscribe` and `unsubscribe last topic -> flood -> subscribe` proving no pre-subscription `Dropped` is emitted.
- [ ] [Review][Patch] Coalesced pending drops can report a fresh cursor with a stale count [crates/daemon/src/api/ws.rs:627] — Within an active coalescing window, `coalesce_decision` only increments `pending_drop_count` and suppresses emission (`crates/daemon/src/api/ws.rs:627-630`). Later, when the window expires, it computes `first_dropped_event_id` from the current `last_delivered_event_id + 1` while `count = pending_drop_count + n` still includes lag bursts that may have occurred before later successful Event deliveries (`crates/daemon/src/api/ws.rs:632-645`). Since `last_delivered_event_id` advances after successful matching Event sends in both normal dispatch paths (`crates/daemon/src/api/ws.rs:295-299`, `crates/daemon/src/api/ws.rs:556-560`), the eventual `DroppedFrame` can describe a range that starts after the presenter's current cursor but whose count includes older suppressed gaps. The changelog says the first/last ids are best-effort, but the frame is still internally misleading and weakens recovery diagnostics. Fix direction: carry a pending dropped range/cursor alongside `pending_drop_count` (for example `pending_drop_first: Option<EventId>`), or clear/rebase pending drops when a normal Event delivery advances the cursor after a suppressed lag. Add a deterministic unit test that suppresses a lag, advances `last_delivered_event_id`, then emits after the window and asserts the count/range remain coherent by the chosen policy.
- [ ] [Review][Patch] AC #1 test does not prove exactly one Dropped frame followed by normal recovery [crates/daemon/tests/contract_daemon.rs:4733] — The acceptance criterion says a blocked reader with capacity 1024 and 1025 envelopes receives exactly one `dropped` frame, then the next frame after `dropped` is the next legitimate event and the socket remains open. The current test publishes 4096 synthetic envelopes (`crates/daemon/tests/contract_daemon.rs:4753-4757`), hunts up to 4200 frames until it sees any `Dropped` (`crates/daemon/tests/contract_daemon.rs:4764-4769`), and only asserts `count >= 1`, `count <= 4096`, and `first <= last` (`crates/daemon/tests/contract_daemon.rs:4770-4778`). It does not assert that there was exactly one `Dropped` in the first lag burst, and the socket-open check only rejects a close frame, so a second `Dropped` immediately after the first would pass (`crates/daemon/tests/contract_daemon.rs:4788-4794`). Fix direction: keep the 4096 smoke test if it is useful for real TCP behavior, but add a tighter contract test around the 1024/1025 boundary or a deterministic harness around the receiver lag path that asserts one Dropped emission for the first burst and that the next matching frame after Dropped is a normal Event/State frame.
- [ ] [Review][Patch] Post-drop delivery test can pass on synthetic flood frames instead of production events [crates/daemon/tests/contract_daemon.rs:4817] — `dropped_frame_keeps_socket_open` floods synthetic events with event ids `1..512` (`crates/daemon/tests/contract_daemon.rs:4817-4822`), then publishes three real events through `publish_via_projection` and records their database-assigned ids (`crates/daemon/tests/contract_daemon.rs:4831-4860`). The read loop accepts any `ServerMessage::Event` whose `event_id` equals one of those three ids (`crates/daemon/tests/contract_daemon.rs:4881-4889`), and the comment assumes the real ids are "well past any synthetic event_id" (`crates/daemon/tests/contract_daemon.rs:4862-4865`). In a fresh database, real ids can overlap the synthetic `1..512` range, so residual synthetic flood frames can satisfy `found_ids == vec![id1, id2, id3]` without proving the post-drop production events were delivered. Fix direction: make synthetic ids disjoint from database ids in this test (for example start at 99_000), or match the post-drop frames by `event_id`, `source`, `session_id == "sess-after"`, and expected `kind` order.
- [ ] [Review][Patch] REST recovery test does not assert the missed persisted events are returned [crates/daemon/tests/contract_daemon.rs:4977] — `dropped_frame_rest_refetch_recovers` records the ids of 40 real, persisted events in `produced` (`crates/daemon/tests/contract_daemon.rs:4977-4993`), then forces lag with 256 synthetic broadcast-only events (`crates/daemon/tests/contract_daemon.rs:4994-4999`). The REST assertion only checks that `/sessions/sess-rest/events?since=<id0>` returns a non-empty response and that `oldest_available_event_id <= id0 + 1` (`crates/daemon/tests/contract_daemon.rs:5027-5037`). It never checks that the persisted `produced` ids are actually present, so the test can pass even if the REST recovery path omits some or all of the events the presenter missed. Fix direction: assert that the REST response contains every expected `produced` id greater than the presenter's authoritative `last_delivered_event_id` (`id0`), in order. If possible, prefer a lag harness where the dropped matching events are also persisted, but at minimum make this test prove the persisted missed set is recoverable.
- [ ] [Review][Patch] AC #3 lacks a deterministic 30-second/default-window policy test [crates/daemon/src/api/ws.rs:991] — Story 2.4's AC #3 sets the sustained-lag bound as `ceil(30s / coalesce_window) <= 31` with the production default `coalesce_window = 1s`. The socket-level test runs for about one second with a 100ms window and allows up to 30 dropped frames (`crates/daemon/tests/contract_daemon.rs:5042-5119`), while the pure policy tests cover short windows but do not simulate the production 30-second horizon (`crates/daemon/src/api/ws.rs:991-1045`). A regression in long-horizon emission-rate bookkeeping could slip through while the integration smoke still passes. Fix direction: add a deterministic pure-policy test that repeatedly calls `coalesce_decision` over a simulated 30 seconds with `Duration::from_secs(1)`, performs the same caller bookkeeping (`last_dropped_at` update and pending reset) after each `Emit`, and asserts the number of emitted Dropped frames is at most 31. Keep the socket test as end-to-end smoke coverage.
...
- `crates/daemon/src/api/ws.rs` — Added per-connection lag state (3 locals: `last_delivered_event_id`, `last_dropped_at`, `pending_drop_count`); replaced `RecvError::Lagged` arm body to call `emit_dropped_or_coalesce`; replaced `TryRecvError::Lagged` arm body in `drain_backlog_under_state` to call the same helper; widened `handle_text_frame` and `drain_backlog_under_state` signatures; added `emit_dropped_or_coalesce` async helper with `#[tracing::instrument(skip_all, fields(n, pending_drop_count, has_cursor))]`. **Code-review fix #1:** introduced `DispatchOutcome { Sent, Filtered, Closed }` enum return type for `dispatch_envelope`; both call sites advance `last_delivered_event_id` only on `Sent` for `Event` envelopes, fixing cursor corruption for state-only and narrow-topic subscribers. **Code-review fix #2:** factored `coalesce_decision` (pure synchronous function returning `CoalesceDecision { Emit, Suppress }`) out of `emit_dropped_or_coalesce` so the policy contract is deterministically unit-testable; added 7 unit tests covering first-lag, cursor-plus-one, within-window suppression, window-expired emit, boundary semantics, repeated-calls accumulation, and singleton lag.
- `crates/daemon/tests/contract_daemon.rs` — Added `coalesce_window: Duration::from_secs(1)` to two existing `WsConfig` constructions (test factory + per-test inline); added `mod story_2_4_dropped { ... }` after `mod story_2_3_snapshot` with 8 contract tests. **Code-review fix #1:** added regression test `state_only_subscriber_does_not_advance_cursor_on_unrelated_events` (9th test) verifying a `state.session.*` subscriber's first `Dropped` frame uses the `EventId(0)` sentinel even after a flood of unrelated Event envelopes.
...
- 2026-05-23 — Addressed code-review findings: (1) cursor-corruption fix via `DispatchOutcome` enum so `last_delivered_event_id` only advances on matching wire-side Event sends + regression test `state_only_subscriber_does_not_advance_cursor_on_unrelated_events`; (2) factored coalescing policy into pure `coalesce_decision` function + 7 deterministic unit tests in `crates/daemon/src/api/ws.rs::tests`. 219 workspace tests pass; clippy clean.
