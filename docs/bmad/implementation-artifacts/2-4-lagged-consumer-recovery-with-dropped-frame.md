...
- [x] [Review][Patch] Lag before any active subscription emits an unsolicited Dropped frame [crates/daemon/src/api/ws.rs:124] — _(full finding text preserved above)_ **Resolved (second round):** Added a `subscriptions.is_empty()` guard in both lag arms (main `rx.recv()` and `drain_backlog_under_state`). When no topic is active the daemon logs at DEBUG and silent-fast-forwards. Two regression tests in `crates/daemon/tests/contract_daemon.rs`: `lag_before_any_subscription_does_not_emit_dropped` (connect → idle flood → subscribe) and `lag_after_last_unsubscribe_does_not_emit_dropped` (subscribe → unsubscribe → flood → re-subscribe). Verified by stash-reverting `crates/daemon/src/api/ws.rs`: the pre-fix test fails with a `{"op":"dropped","count":252,...}` frame arriving during the wait-subscribe-live probe phase.
- [x] [Review][Patch] Coalesced pending drops can report a fresh cursor with a stale count [crates/daemon/src/api/ws.rs:627] — _(full finding text preserved above)_ **Resolved (second round):** Added `pending_dropped_first: Option<EventId>` alongside `pending_drop_count` in the coalescing state. On the FIRST suppression of a window (when `pending_drop_count` was 0), `coalesce_decision` captures the gap origin (`last_delivered + 1` or `EventId(0)` sentinel) into `pending_dropped_first`. Subsequent suppressions in the same window do NOT overwrite it. On the eventual Emit, `first_dropped_event_id` uses the captured value rather than the (possibly cursor-advanced) current value; the caller resets both `pending_drop_count = 0` and `*pending_dropped_first = None` after a successful wire send. Deterministic unit test `coalesce_emit_after_cursor_advance_uses_captured_first` walks the exact scenario: lag → emit → cursor advance → suppression with captured first → cursor advance again → window expiry emits with the originally-captured first (asserts `Emit { count: 9, first: EventId(21), last: EventId(29) }` not the pre-fix `Emit { first: EventId(41), ... }`). All six pre-existing `coalesce_decision` unit tests updated for the new signature.
- [x] [Review][Patch] AC #1 test does not prove exactly one Dropped frame followed by normal recovery [crates/daemon/tests/contract_daemon.rs:4733] — _(full finding text preserved above)_ **Resolved (second round):** Two-pronged fix per the reviewer's suggestion. (a) Tightened the e2e test `dropped_frame_after_1025_envelopes_with_blocked_reader` to assert "the next frame after Dropped is a legitimate Event or State (NOT another Dropped, NOT Close)" — panics with "AC #1 violation: a second Dropped frame followed the first within the coalesce window" if a second Dropped sneaks in. (b) Added pure-policy unit test `coalesce_single_lag_burst_produces_exactly_one_emit` documenting the structural one-emit-per-burst guarantee at the policy level (`coalesce_decision` is only called from `Lagged` arms; one `Lagged(n)` arrival → one decision → one Emit).
- [x] [Review][Patch] Post-drop delivery test can pass on synthetic flood frames instead of production events [crates/daemon/tests/contract_daemon.rs:4817] — _(full finding text preserved above)_ **Resolved (second round):** Synthetic flood ids shifted from `1..=512` to `99_000+1..=99_512` (well above any DB-assigned id this test produces); the read loop's match strengthened from "any id matches one of {id1,id2,id3}" to "source == 'claude' AND session_id == 'sess-after' AND id matches AND EventKind order matches `[PreToolUse, PostToolUse, PreToolUse]`." A residual synthetic flood envelope can no longer satisfy the assertion.
- [x] [Review][Patch] REST recovery test does not assert the missed persisted events are returned [crates/daemon/tests/contract_daemon.rs:4977] — _(full finding text preserved above)_ **Resolved (second round):** `dropped_frame_rest_refetch_recovers` now extracts `expected_ids: Vec<EventId>` from the 40 produced events (filtered to `id > id0`) and asserts `returned_ids == expected_ids` (full equality including order), not just "non-empty response." A partial REST response or out-of-order return would now fail loudly with a specific diff.
- [x] [Review][Patch] AC #3 lacks a deterministic 30-second/default-window policy test [crates/daemon/src/api/ws.rs:991] — _(full finding text preserved above)_ **Resolved (second round):** Added pure-policy unit test `coalesce_30s_horizon_with_1s_window_bounded_at_31_emits` that simulates 30 virtual seconds (300 calls to `coalesce_decision` at 100ms intervals) with the production default `Duration::from_secs(1)` window. Mirrors the caller's post-Emit bookkeeping (reset pending, capture last_dropped_at, advance cursor). Asserts `emit_count <= 31` AND `emit_count >= 1`. Independent of scheduler / TCP / real wall clock — would catch a long-horizon bookkeeping regression that escapes the wall-clock smoke test.
...
- 2026-05-23 — Addressed second-round code-review findings (six patch-level): (1) `subscriptions.is_empty()` guard in both lag arms → silent fast-forward for pre-subscription lag, with two new e2e regression tests; (2) `pending_dropped_first: Option<EventId>` captured at first suppression of a window so the eventual Emit reports the original gap origin even after cursor advance, with a deterministic unit test; (3) AC #1 e2e test tightened to assert the next frame after Dropped is a legitimate Event/State (not another Dropped, not Close) + new unit test documenting structural one-emit-per-burst; (4) synthetic flood ids shifted to `99_000+` and post-drop match strengthened to (source, session_id, event_id, kind); (5) REST recovery test asserts full equality of returned vs expected id list (preserves order); (6) deterministic 30s/1s-window pure-policy test asserting Emit count ≤ 31. 224 workspace tests pass; clippy clean.
