...
- `crates/daemon/src/db/queries.rs` — `INSERT_EVENT` + 3 SELECTs carry `cwd`.
- `crates/daemon/src/projection/state.rs` — `transition` gains `cwd` param + carry-forward, `started_at` set-once; new unit tests.
- `crates/daemon/src/projection/session.rs` — `write_inner` threads `cwd`; `rebuild_missing_projections` reads/threads `cwd` (+ `RebuildEventRow` alias); sentinel writers pass `cwd = NULL`.
...
- `docs/decisions/0006-session-cwd-on-the-wire.md` (pre-authored 2026-06-01; unchanged this PR).
