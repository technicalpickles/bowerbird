...
- `crates/daemon/src/db/queries.rs` — `INSERT_EVENT` + 3 SELECTs carry `cwd`; `SELECT_MIN_CREATED_AT_FOR_SESSION` added (review: legacy `started_at` backfill).
- `crates/daemon/src/projection/state.rs` — `transition` gains `cwd` param + carry-forward, `started_at` set-once; new unit tests.
- `crates/daemon/src/projection/session.rs` — `write_inner` threads `cwd`; `rebuild_missing_projections` reads/threads `cwd` (+ `RebuildEventRow` alias); sentinel writers pass `cwd = NULL`. Review: `write_inner` backfills legacy `started_at` from `MIN(events.created_at)` when the prior state's `started_at` is `None`.
...
**Tests:**
- `crates/protocol/tests/contract_protocol.rs` — additive-compat + old-blob-reads-back + round-trip.
- `crates/daemon/tests/contract_daemon.rs` — migration v3, rebuild, REST cwd/started_at, constructor audit.
