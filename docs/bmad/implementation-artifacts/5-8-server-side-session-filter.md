...
- [x] **Task 5: WS — thread `states` into the snapshot path** (AC: #10, #11)
  ...
  - [x] Check the `Unsubscribe` arm — it does NOT gain `states` (it has no snapshot); leave `ClientMessage::Unsubscribe { topic }` as-is. Only `Subscribe` carries the snapshot filter.
...
- [x] **Task 11: Manual smoke against the running daemon** (AC: #2, #3, #5, #6, #10)
...
- **WS `states` parse:** the field is `Vec<String>` on the wire; the daemon joins a non-empty `states` into a CSV and feeds it to the SAME `parse_state_filter` the REST `?state=` uses (empty vec short-circuits to unfiltered, since `parse_state_filter("")` is an error by design). One token table, one validation path.
...
- `crates/daemon/src/projection/snapshot.rs` — `snapshot_for_topic` gains `state_filter: &[SessionCurrentState]` param + read-derived skip (+ 3 filter tests; existing callers pass `&[]`).
- `crates/daemon/src/api/ws.rs` — `Subscribe` arm destructures `states`, parses via `parse_state_filter` (bad token → `close_with_bad_message`), threads `&state_filter` into `snapshot_for_topic`.
...
- `crates/daemon/tests/contract_daemon.rs` — `story_5_8_session_filter` REST module (9 tests), `story_2_1_ws::ws_invalid_states_token_closes_with_policy_violation`, `story_2_3_snapshot::snapshot_states_filter_excludes_ended_live_transition_still_arrives`.
