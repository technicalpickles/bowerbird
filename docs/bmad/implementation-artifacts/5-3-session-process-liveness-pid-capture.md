...
- [x] [Review][Decision] PID-only liveness lacks process identity — `crates/daemon/src/projection/liveness.rs:65` uses only `kill(pid, 0)`, which proves that some process currently has that PID, not that it is the original Claude session. PID reuse can keep a dead session non-`Ended`; wrapper/reparent scenarios can point `bowerbird_ppid` at the wrong long-lived parent. Decide whether V1 accepts this known PID-only risk, defers process-birth validation, or stores/checks an OS-specific process identity marker. **Resolved 2026-05-28:** V1 accepts the risk and defers the process-birth marker (`bowerbird_pstart` via Linux `/proc/<pid>/stat` field 22 / macOS `kinfo_proc.p_starttime`) to a follow-up. Mitigation in V1: every real hook event re-binds `last_pid` via overwrite-on-Some carry-forward, so a still-firing session stays correctly tracked. Documented in `docs/protocol.md` §"V1 PID-only liveness: known limitations" and `deferred-work.md` entry #7 under Story 5.3.
- [x] [Review][Patch] Stale liveness snapshots can end a resumed live session [crates/daemon/src/projection/liveness.rs:128] — `probe_once` selects projection rows, drops the DB connection, then later writes synthetic `SessionEnded` from that stale `stored` snapshot. If a real hook arrives between the SELECT and synthetic write, the probe can flip the revived session back to `Ended`; with an old dead PID it can also overwrite the newer live `last_pid`. Re-check the latest projection state/liveness immediately before emitting, or make the synthetic write conditional on the row still matching the probed state. **Resolved:** new `projection::session::write_if_state_matches(envelope, WritePrecondition { expected_current_state, expected_last_pid })` re-reads the projection row inside the writer txn and only commits the synthetic write if `current_state` AND `last_pid` still match the snapshot — otherwise the txn drops without committing, no broadcast is emitted, and `Ok(None)` flows back through `ProbeReport.skipped_stale`. Regression: `liveness_probe_skips_stale_row_when_real_hook_interleaved` in `crates/daemon/tests/contract_daemon.rs`.
- [x] [Review][Patch] Startup liveness cleanup can fail open or under-report failed rows [crates/daemon/src/main.rs:189] — AC #10 depends on the synchronous startup probe completing before WS bind, but `main.rs` logs `probe_once` errors and continues to bind, and `liveness.rs:171` logs per-row `write(SessionEnded)` failures while still returning `Ok(emitted)`. Presenters can receive stale startup snapshots after failed cleanup, and logs may say no dead sessions were found when writes actually failed. Surface failed rows through the probe result and make the startup guarantee explicit. **Resolved:** `probe_once` now returns `Result<ProbeReport>` with `{emitted, failed, skipped_stale}`. `main.rs` startup-probe handler logs at WARN with full counts when any row write failed so a partial cleanup is observable. Hard-fail-on-failed-rows behavior left to a future config flag — tracked in `deferred-work.md` entry #8 under Story 5.3.
- [x] [Review][Patch] `bowerbird_ppid = 0` is accepted and probes the process group [crates/adapter-claude/src/normalize.rs:97] — normalize accepts `0` as `Some(0)`, and `liveness.rs:75` calls `kill(0, 0)`, which has process-group semantics rather than single-process semantics. A malformed/custom producer can therefore make a session appear alive indefinitely. Treat PID 0 as invalid (`None` or dead) before it reaches `kill`. **Resolved:** `normalize.rs` filters PID 0 to `None` via `.filter(|n| *n != 0)`. Regression: `normalize_rejects_bowerbird_ppid_zero` in `crates/adapter-claude/tests/contract_adapter.rs`. Behavioral changelog entry updated to list `0` alongside missing / non-integer / out-of-range.
- [x] [Review][Patch] Liveness shutdown cancellation does not stop an in-flight probe iteration [crates/daemon/src/projection/liveness.rs:205] — `run` checks `shutdown.cancelled()` only before awaiting `probe_once`; once a probe starts it can continue writing `SessionEnded` events after shutdown is requested. Task 7 explicitly says not to finish an in-flight iteration during shutdown. Pass cancellation into the per-row loop or check it between rows before synthetic writes. **Resolved:** `probe_once` now takes `&CancellationToken` and checks `shutdown.is_cancelled()` before every per-row write. An in-flight iteration that observes cancellation returns the partial `ProbeReport` and exits. Regression: `liveness_probe_observes_shutdown_mid_iteration` in `crates/daemon/tests/contract_daemon.rs`.
- [x] [Review][Patch] Missed-tick/no-queue behavior lacks the required deterministic test [crates/daemon/tests/contract_daemon.rs:7936] — Task 9 requires `liveness_probe_missed_tick_does_not_queue` using paused Tokio time, but the new liveness tests only exercise `probe_once`. The dead-PID test also uses `std::thread::sleep` at `contract_daemon.rs:8052`, contrary to the deterministic timing-test discipline. Add the skipped-tick regression test and remove the real sleep. **Resolved:** extracted `liveness::run_loop` as a testable seam; new `liveness_probe_missed_tick_does_not_queue` test uses `#[tokio::test(start_paused = true)]` + `tokio::time::advance` to assert one tick fires (not three queued) after advancing through three 5s boundaries. The dead-PID test no longer calls `std::thread::sleep` — `wait()` reaps the child synchronously, so the PID is dead the instant `wait()` returns. Required adding the `test-util` feature to tokio dev-dep in `crates/daemon/Cargo.toml`.
- [x] [Review][Patch] AC #17 compatibility tests miss the WS frame dispatch paths [crates/protocol/tests/contract_protocol.rs:472] — AC #17 names `SessionState`, `StateFrame`, and `EventFrame` decode paths. Current tests decode local legacy `SessionState` and `Event` structs, but do not cover legacy `StateFrame`, `EventFrame`, or `ServerMessage::{State,Event}` dispatch with the new `last_pid`, `pid`, `Ended`, and `SessionEnded` fields/variants. Add nested-frame compatibility tests so the actual WS payload shape is pinned. **Resolved:** four new additive-compat tests in `crates/protocol/tests/contract_protocol.rs` — `additive_compat_state_frame_with_ended_decodes_through_server_message`, `additive_compat_event_frame_with_session_ended_decodes_through_server_message`, `additive_compat_event_frame_with_session_ended_decodes_directly`, `additive_compat_state_frame_with_ended_decodes_directly`. A shared `legacy_v1_0_mocks` module supplies the v1.0-shaped types so each test exercises the actual nested-frame decode path.
...
- **2026-05-28 review-fix iteration — intermittent workspace-test hang on `liveness_probe_skips_stale_row_when_real_hook_interleaved`.** Two consecutive `cargo test --workspace -- --test-threads=1` invocations parked on this test for 8+ and 14+ minutes (contract_daemon process in `S` state, sub-1s CPU). The third invocation completed normally (389/389 in normal time). The test passes in isolation (0.01s) and as part of `story_5_3_liveness::` alone (9/9 in 0.04s). It's the only test in the suite that exercises the SKIP branch of the new `write_if_state_matches` (the Ok(None) return-without-commit path). Not yet root-caused — captured for follow-up as bean `gt-hz99` with full observations, hypotheses (rusqlite tx-drop rollback wedge after a prior test leaves the writer-pool conn in some specific state; macOS FD/WAL handle accumulation across the 144 prior tests; orphan child process from SIGKILL tests holding a resource), and an investigation plan (repro, lldb sample, RUST_LOG=trace deadpool, consider explicit `tx.rollback()` in the skip branch). **NOT a regression of the original story-5.3 probe-conn deadlock** — that was probe_once holding a conn across per-row writes, fixed by scoping the SELECT in a block; the new pattern reuses that scoping correctly.
...
- 2026-05-28 — Addressed seven code-review findings: PID-only V1 limitation documented + carry-forward hardened (finding #1 — deferred process-birth marker); writer-txn precondition + `write_if_state_matches` for stale-snapshot race (#2); `ProbeReport { emitted, failed, skipped_stale }` surfaces probe failures from startup probe (#3); PID 0 filtered at adapter boundary (#4); `&CancellationToken` threaded into `probe_once` per-row loop (#5); deterministic `liveness_probe_missed_tick_does_not_queue` test + removed `std::thread::sleep` from dead-PID test (#6); four new WS-frame additive-compat tests for `ServerMessage`/`StateFrame`/`EventFrame` dispatch (#7). Two new entries in `deferred-work.md` (process-birth marker; hard-fail-on-failed-rows config flag). Workspace tests + fmt + clippy green. Status → review.
