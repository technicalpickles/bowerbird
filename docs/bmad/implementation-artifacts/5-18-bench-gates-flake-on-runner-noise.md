...
| Date | Author | Summary |
|------|--------|---------|
| 2026-07-31 | claude-fable-5 | Daemon macOS `regression_max_ratio` 1.30 → 2.0: the review decision's pre-agreed trigger fired the same day it was recorded. Run `30632168195` (daemon-neutral diff) failed both best-of-2 attempts (burst +51%, fanout3 +60% over worst x 1.30), confirming the clustering finding about the 7-minute calibration window. Seeds unchanged per the do-not-chase anti-pattern; ratio derived from measured cross-window spread. CI run `30632592030` fully green after the change. |
