...
- `crates/shim/src/main.rs` (modified) — `use std::io::{self, Read, Write}`; stderr line in `Err` arm + `resolve_log_path()` failure branch.
- `crates/shim/tests/contract_shim.rs` (modified) — stderr assertions on connect-refused (named line) + 503/400 (empty, NFR20 guard).
