...
1. **AC1 — daemon-down names the cause on stderr.** When the shim fails to connect to the ingest socket (`Error::Connect`, daemon unreachable), it writes exactly one line to **stderr** of the form `bowerbird: daemon not running, event dropped (see <log-path>)`, where `<log-path>` is the resolved shim-log path (default `~/.bowerbird/shim.log`, or `BOWERBIRD_SHIM_LOG` when set). The existing ERROR line in the file log is still written, and the exit code stays **1** (NFR20 contract intact — `Error::Connect → exit 1`).

2. **AC2 — every exit-1 variant names a cause; exit-0 stays stderr-silent.** Every error whose `exit_code()` is `1` (the ERROR-level class: `Stdin`, `StdinEmpty`, `StdinNotJsonObject`, `StdinTooLarge`, `StdinJson`, `Connect`, `LogIo`, `BadArgs`, `NoHome`) emits one `bowerbird: <cause> (see <log-path>)` stderr line. Every error whose `exit_code()` is `0` (the WARN-level class: `SocketIo`, `BadResponse`, `Backpressure`, `Backpressure503`, `DaemonError400`) writes **nothing** to stderr — the daemon is up and answering, fire-and-forget per NFR20, so Claude must see success, not a warning.
...
- `main`'s `Err(e)` arm now emits `bowerbird: {hint} (see {log_path})` to stderr when `stderr_hint()` is `Some`, after the existing `log::append`. The pre-`run` `resolve_log_path()` failure branch emits `bowerbird: HOME not set, cannot record event` (no `(see ...)` pointer — no log path resolved). Both writes use `let _ = writeln!(...)` so a failed stderr write is swallowed exactly like the log append (AC5); never panics, never changes the exit code.
...
- `crates/shim/src/main.rs` (modified) — `use std::io::{self, Read, Write}`; stderr line in `Err` arm + `resolve_log_path()` failure branch. Pass-1: capture `log::append` result and drop the `(see <log>)` pointer when it fails; new `one_line_path()` helper sanitizes the log path; `#[cfg(test)] mod tests` with 3 `one_line_path` unit tests.
- `crates/shim/tests/contract_shim.rs` (modified) — stderr assertions on connect-refused (named line) + 503/400 (empty, NFR20 guard). Pass-1: connect-refused assertion tightened to exact-match; added `shim_omits_log_pointer_when_log_append_fails`, `shim_names_cause_when_no_home_and_no_log_path`, `shim_stderr_stays_one_line_with_newline_in_log_path`.
