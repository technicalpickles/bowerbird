...
- [x] [Review][Decision] Existing false `WaitingInput` rows are not remediated by this branch — The one-arm change only affects future projections. Startup calls `rebuild_missing_projections` (`crates/daemon/src/main.rs:166`), but that path rebuilds only sessions with no projection row and skips existing rows (`crates/daemon/src/projection/session.rs:443`, `crates/daemon/src/projection/session.rs:477`, `crates/daemon/src/projection/session.rs:484`). The new `IdlePrompt` behavior also deliberately preserves prior `WaitingInput` (`crates/daemon/src/projection/state.rs:80`, `crates/daemon/src/projection/state.rs:83`, `crates/daemon/src/projection/state.rs:88`; locked by `transition_notification_idle_prompt_prior_waiting_input_preserved`). That means live sessions already misclassified under the old `idle_prompt -> WaitingInput` rule can stay stuck after deploy unless a non-idle hook changes them, a one-time repair/reprojection is added, or the story explicitly accepts a future-event-only fix. This conflicts with the proposal success criterion that the live-idle wall drains (`docs/bmad/planning-artifacts/sprint-change-proposal-2026-05-29-idle-prompt-reclassification.md:148`) and needs a maintainer decision because AC #7 also says no migration/no new field.
  - **Maintainer decision (pickles, 2026-05-29): targeted startup repair.** Added `projection::session::repair_idle_prompt_waiting_input`, called at daemon startup after `rebuild_missing_projections` and before the liveness probe / WS bind. For each existing `WaitingInput` row it re-folds the post-5.6 `transition` over the session's event log (shared `recompute_state_from_log` helper, extracted from `rebuild_missing_projections` so the two replay paths can't drift) and rewrites only when the result changes — so the idle_prompt-stuck rows drain (normally → `Idle`) while genuine `permission_prompt`/`elicitation_dialog` blocks recompute back to `WaitingInput` and are left untouched. Idempotent (writes only on change). **Data reprojection, not a schema migration** — no SQLite migration, no new column, no protocol change, so AC #7 still holds. Covered by `repair_drains_stale_idle_prompt_waiting_input` (drains + idempotency) and `repair_preserves_genuine_permission_block` in `contract_daemon.rs`. ADR 0005 Consequences + the 5.6 changelog entry updated to record it.
...
- **`WaitingInput` is now narrowed to two types:** `permission_prompt` and `elicitation_dialog` (incl. `AskUserQuestion`) are the only `notification_type` values that *transition a session into* it. `idle_prompt` preserves prior state, so a session already in `WaitingInput` stays there on a later idle nudge — the narrowing is about transitions *into* `WaitingInput`, not a hard "`idle_prompt` ⇒ never `WaitingInput`" guarantee.
...
- `docs/bmad/planning-artifacts/epics.md` (modified — inserted/resequenced the Story 5.6 section; added superseded/extended notes to the Story 5.3 notification-type ACs)

**Code-review follow-up edits (post-review):**
- `docs/presenter-authoring.md` (modified — `WaitingInput` rendering guidance narrowed to `permission_prompt`/`elicitation_dialog`; `idle_prompt` documented as transient)
- `crates/daemon/src/projection/session.rs` (modified — extracted shared `recompute_state_from_log` + `parse_notification_type` helpers from `rebuild_missing_projections`; added `repair_idle_prompt_waiting_input` one-time startup repair)
- `crates/daemon/src/main.rs` (modified — call `repair_idle_prompt_waiting_input` at startup after rebuild, before the liveness probe / WS bind)
- `crates/daemon/tests/contract_daemon.rs` (modified — two stale doc-comments corrected to the post-5.6 buckets [no assertion change]; added `repair_drains_stale_idle_prompt_waiting_input` and `repair_preserves_genuine_permission_block` + two test helpers)
- `docs/decisions/0005-idle-prompt-transient-not-input-required.md` (modified — Consequences records the one-time startup repair)
...
- 2026-05-29 — Code review: three `[Patch]` findings resolved (stale presenter/contract/epics docs; overstated invariant wording; File List). One `[Decision]` finding resolved per maintainer choice (pickles) with a **targeted one-time startup repair** (`repair_idle_prompt_waiting_input`) that drains pre-5.6 idle_prompt `WaitingInput` rows by re-folding the post-5.6 `transition` over the event log, without a schema migration; +2 contract tests (487 workspace tests green, fmt + clippy clean).
