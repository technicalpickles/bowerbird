...
- [x] [Review][Patch] Snapshot read failure silently leaves the client unsubscribed [crates/daemon/src/api/ws.rs:383] — **Resolved by reverting to the spec's `log + insert empty` policy.** The previous round's resolution (return early without inserting the topic) was the source of this regression — it traded the snapshot retry-poison concern for a worse silent-unsubscribed bug. Subscribe arm now logs the error, emits `Vec::new()`, and still calls `subscriptions.insert(t)` so the live broadcast path delivers `State` frames for the lifetime of the connection. Snapshot retry is via reconnect (the canonical WS retry mechanism and the only path that gets a fresh `pre_existing` set). The trade-off is documented in the Subscribe-arm header so a future reviewer does not re-introduce the previous-round bug. New contract test `snapshot_read_failure_keeps_live_subscription_active` forces the reader pool closed before Subscribe, asserts the subscription is live via `wait_subscribe_live`, then proves a subsequent `publish_via_projection` reaches the wire. — The current failure path catches `snapshot_for_topic` errors, logs `snapshot_for_topic failed; subscribe NOT applied`, returns `true`, and never inserts the parsed topic into `subscriptions` (`crates/daemon/src/api/ws.rs:383-401`; insertion is only later at `crates/daemon/src/api/ws.rs:408`). The WebSocket remains open, and the protocol has no subscribe ack/error frame, so a valid client cannot distinguish this from a successful subscribe with an empty snapshot. Future live `State` frames for that topic will not be delivered. This also deviates from the story's Task 2 reliability policy, which says snapshot errors should log, emit zero snapshot frames, and keep the connection live rather than losing the subscription. Fix direction: make the failure client-visible or keep live delivery intact. The least invasive patch is to log the snapshot error, substitute `Vec::new()`, continue to `subscriptions.insert(t)`, and add a focused contract test that forces reader-pool/interact failure during subscribe and then proves a subsequent live publish is delivered. If retryability is preferred instead, close the socket with a retryable policy/error reason rather than returning `true` on an apparently healthy but unsubscribed connection.
- [x] [Review][Patch] Large snapshot emission can cause silent live-frame loss under broadcast lag [crates/daemon/src/api/ws.rs:408] — **Pushback — Story 2.4 scope.** The lag-policy change the reviewer proposes (close-on-`RecvError::Lagged`, emit a sync/dropped signal, or chunk-with-interleave) is exactly the scope of Story 2.4 (lagged-consumer recovery with `DroppedFrame`). Story 2.3 explicitly preserves the Story 2.1 WARN-log-and-continue lag behavior — Dev Notes line 411: "no new lag policy is added." Changing it here would either double-implement what 2.4 will land or pre-commit to a recovery choice without 2.4's full design. For the documented single-developer N≤100 workload the entire snapshot completes in single-digit milliseconds (story Latency section), so the lag-during-snapshot scenario is theoretical at today's scale. A new deferred-work entry under "Deferred from: code review of story-2.3" ties the stress test the reviewer asks for to 2.4's recovery test suite, so the scenario stays first-class on the backlog. — The Subscribe arm inserts the new topic before sending every snapshot frame (`crates/daemon/src/api/ws.rs:405-408`), then serially serializes and sends the whole snapshot on the same connection task (`crates/daemon/src/api/ws.rs:412-427`). While that loop is running, the task is not polling `rx.recv()`. If a slow client subscribes to `state.session.*` with many existing sessions and more than the broadcast capacity worth of live updates arrives during the snapshot, this receiver can lag; the current lag path only logs and continues (`crates/daemon/src/api/ws.rs:262-265`). The client then misses live frames after the snapshot without any recovery signal, undermining the story's "snapshot before subsequent live frame" intent for busy daemons. Fix direction: choose an explicit recovery behavior for lag introduced during snapshot delivery. Practical options are: close the connection on `RecvError::Lagged` so the client reconnects and gets a fresh snapshot; emit a recovery/sync/dropped signal if that is in scope; or bound/chunk snapshot delivery so the task can interleave live receive, shutdown, and pong handling. Add a stress test with a small `ws_broadcast_capacity`, a large snapshot, and concurrent live publishes that currently triggers `RecvError::Lagged`, then assert the chosen recovery behavior is observable by the client.
- [x] [Review][Patch] Negative WebSocket tests can false-pass on delayed forbidden frames [crates/daemon/tests/contract_daemon.rs:4235] — **Resolved by replacing both fixed-timeout windows with publish-order barriers.** In `specific_id_subscription_excludes_other_sessions`, the 300ms `tokio::time::timeout` is gone; instead the test publishes the forbidden `sess-B` envelope, then a permitted `sess-A` live update, then reads the next State frame and asserts it is `sess-A` (with `PostToolUse` last_event_kind to distinguish the live frame from the prior snapshot). Because `tokio::sync::broadcast` preserves per-channel publish order, a buggy `Topic::matches` that leaked `sess-B` would deliver `sess-B` first — surfacing as the assertion fail. In `overlapping_subscriptions_do_not_re_snapshot`, the 200ms trailing timeout is removed; `wait_subscribe_live` (already in the test) is the deterministic barrier — it panics on any non-probe frame during readiness, so a duplicate snapshot for `sess-A` would surface immediately. The subsequent live publish + read additionally pins `last_event_kind == PostToolUse`, which differs from the snapshot's `PreToolUse`, so any leaked snapshot frame would surface as a kind mismatch rather than as silent timeout flake. Both fixes are deterministic across CI load, scheduler jitter, and TCP backpressure. — `specific_id_subscription_excludes_other_sessions` treats "no frame within 300ms" as proof that `sess-B` did not leak to a `state.session.sess-A` subscriber (`crates/daemon/tests/contract_daemon.rs:4224-4238`). `overlapping_subscriptions_do_not_re_snapshot` uses the same pattern with a 200ms timeout for trailing frames (`crates/daemon/tests/contract_daemon.rs:4450-4455`). Under slow CI, scheduler delay, or a backed-up socket, an incorrectly delivered frame could arrive after the timeout and the test would pass. Fix direction: replace fixed negative sleeps with an ordering barrier. For the specific-session case, publish the forbidden `sess-B` update, then publish a permitted `sess-A` probe/update and read until the permitted frame is observed, failing if any forbidden/non-probe frame appears before it. For the overlapping-subscription case, send a permitted live update after the second subscribe and assert the next non-probe frame is that live update, with no snapshot-shaped frame appearing before the barrier.
...
- Second-round review (2026-05-22): three new patch-level findings landed. Subscribe arm reverted to "log + insert empty" on snapshot failure — the first round's "don't insert" approach introduced a silent-unsubscribed regression because the V1 protocol has no Subscribe ack/error frame, so a failed snapshot left the client unable to receive live `State` frames at all. Trade-off (snapshot retry must go through reconnect rather than re-Subscribe on the same connection) is documented in the Subscribe-arm header so a future reviewer does not re-introduce the previous-round bug. Negative-window timeouts in two contract tests replaced with publish-order barriers (broadcast-order-deterministic for the specific-id case; `wait_subscribe_live` + `last_event_kind` distinction for the overlapping-subscribe case). Lag-during-snapshot deferred to Story 2.4 with a fresh deferred-work entry; pushback rationale: lag policy is 2.4's scope and the lag-during-snapshot scenario is theoretical at single-developer N≤100 workloads.
...
- `docs/bmad/implementation-artifacts/deferred-work.md` (modified) — struck through the Epic-1-retro `SyncFrame` invariant note with a backlink to this story; added a new "Deferred from: code review of story-2.3" section pointing Story 2.4 at the lag-during-snapshot scenario.
...
| 2026-05-22 | Addressed second-round code-review findings. Subscribe arm reverted to spec's `log + insert empty` policy on snapshot failure — the previous round's "return without insert" introduced a silent-unsubscribed bug because V1 has no Subscribe ack/error frame; live broadcast now keeps flowing even after a snapshot read error. New contract test `snapshot_read_failure_keeps_live_subscription_active` closes the reader pool before Subscribe, then proves a subsequent `publish_via_projection` reaches the wire. Negative-test fixed-timeout windows replaced with publish-order barriers in `specific_id_subscription_excludes_other_sessions` (publish sess-B then sess-A live update; broadcast order is deterministic) and `overlapping_subscriptions_do_not_re_snapshot` (drop the 200ms trailing window; rely on `wait_subscribe_live` and a `last_event_kind` distinction between live PostToolUse and snapshot PreToolUse). One finding pushed back with technical reasoning: lag-during-snapshot recovery is Story 2.4's stated scope and theoretical at N≤100; new deferred-work entry under "Deferred from: code review of story-2.3" ties the suggested stress test to 2.4's recovery suite. 198 workspace tests pass (197 + 1 new failure-keeps-live test). |
| 2026-05-23 | Status moved to `done` after second-round review responses landed; PR #25 marked ready for review. |
