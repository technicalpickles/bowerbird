...
- [x] **Task 9: CI wiring for per-platform bench gate** (AC: #1)
  ...
  - [x] Commit baseline files under `crates/shim/benches/baselines/macos.json` and `…/linux.json` after the first green run on each platform — the dev should run the bench locally to seed the baselines, then update them after CI's first run to match the hosted-runner numbers (these are the authoritative baselines). **Status:** `linux.json` seeded locally from this session's bench run; `macos.json` will be seeded after CI's first run on macOS-latest via the artifact upload path documented in `crates/shim/benches/README.md`.
...
- [x] **Task 10: Final checks**
  ...
  - [x] `cargo bench -p bowerbird-shim` — produces a baseline locally; commit the result under `crates/shim/benches/baselines/<platform>.json` (or wait for CI to seed; either works for the first run). **Linux baseline committed** at `crates/shim/benches/baselines/linux.json`.
