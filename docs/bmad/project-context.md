...
**Additive v1.x outbound enum value (opaque-string variant):** the `SessionEnded` payload's `reason` gained `pid_superseded` (Story 5.11, ADR 0009) alongside `pid_dead` / `no_pid_at_upgrade`. `reason` is an opaque JSON string *inside* the event `payload`, not a typed protocol field — so there is no `crates/protocol/src` change and no migration; the CI changelog gate (which fires on `protocol/src` diffs) is not triggered, and the `type: behavioral` changelog entry is added by hand for the documentation contract. Presenters treat `reason` as opaque and special-case only the values they render distinctly; an unrecognized reason is forward-compatible by construction.
...
**Shim logging exception:** the shim does not log to stdout/stderr on the success path or the exit-0 (daemon-answered) paths (Claude's hook environment shouldn't see daemon-internal noise), but it should log to a file (e.g. `~/.bowerbird/shim.log` with rotation). Failures must be visible somewhere; "silently exits on error" makes the shim untestable in the field. On an exit-1 failure the shim additionally emits exactly one `bowerbird: <cause>` line to stderr (with a fixed `(see the shim log)` pointer — never the env-controlled path — only when the log append succeeded) so a daemon outage renders as a named cause rather than Claude's causeless `No stderr output` (Story 5.10).
...
- **Rolled-over predecessor sessions linger on a still-live PID** (dogfood finding 2026-06-11, bean `gt-e9dc`). One live `claude` PID hosts many session_ids over its life (`/clear`, `claude --resume`, compaction roll the session_id forward, keep the OS process), so the PID-death liveness probe never ends a rolled-past predecessor — its `last_pid` is still alive. The daemon ends predecessors **event-driven**: when a successor session emits on PID `P`, every *other* non-`Ended` session still claiming `P` is ended with `SessionEnded { reason: pid_superseded }` (Story 5.11 / ADR 0009, which extends ADR 0004 one step earlier than PID death). This is a **best-effort post-commit side effect** of the successor's write — deliberately *not* folded into the successor's transaction (preserves the two-writes-per-txn invariant; a victim-end failure never fails the successor's already-durable write). **Residual risk, accepted for v0.1.0:** if the daemon crashes (or the follow-up pool/SELECT/clock step fails) between the successor's durable commit and the supersession write, the predecessor stays live-looking until the successor's *next* PID-carrying event re-runs supersession, or until `P` itself dies and the probe reaps the predecessor via `pid_dead`. The window is bounded (a `claude` PID does not live forever), so there is no truly-indefinite leak. A durable startup/probe co-PID reconciliation pass — closing the window faster and sweeping already-stranded predecessors — is deferred (bean `gt-suf7`), out of Story 5.11 scope per the "no retroactive gc" no-list. `/replay` does **not** run supersession: the synthetic `SessionEnded` rows are already in the log being replayed, so re-running it would double-generate them and could end the current live holder on replay arrival order (Story 5.11 review finding #2).
- **Daemon down after a reboot drops every event until restart** (dogfood Finding 1, 2026-06-01). On **macOS** the daemon is supervised by a launchd LaunchAgent (`com.technicalpickles.bowerbird.daemon`) installed by `bowerbird install`: `RunAtLoad=true` brings it back on login/reboot and `KeepAlive={SuccessfulExit=false}` restarts it on crash (non-zero exit). `bowerbird stop` (macOS) boots the loaded LaunchAgent out (`launchctl bootout`), so launchd has nothing left to restart — a forced, SIGKILL-escalated stop is no longer bounced back by `KeepAlive` (the daemon's exit code is no longer the stop guarantee). PID-file SIGTERM/SIGKILL is the manual / Linux / unverifiable-launchctl fallback. Per ADR 0007 (Story 5.9 review pass-6). On **Linux**, supervision stays manual for V1 (systemd deferred); the shim still never blocks Claude, so a down daemon costs events, not the coding session.
...
- **Logging on failure goes to a file**, not stdout/stderr, on the success and exit-0 (daemon-answered) paths. The shim runs inside Claude's hook environment — anything on stdout/stderr on those paths risks polluting Claude's experience. Proposed path: `~/.bowerbird/shim.log` with size-based rotation. **Scoped exception (Story 5.10):** on an exit-1 failure the shim emits exactly one `bowerbird: <cause>` line to stderr (a fixed `(see the shim log)` pointer — never the env-controlled path — appended only when the log append succeeded). Claude already surfaces a hook error on a non-zero exit, so this replaces its causeless `No stderr output` with a named cause without polluting the success path.
...
- **State emission and event INSERT must happen in the same SQLite transaction.** Otherwise a subscriber gets a state-change notification for an event that hasn't landed yet, and a presenter that re-snapshots via REST sees inconsistent data. Validate with a fault-injection test (SIGKILL during a load run; on restart, projection must match event log). Story 5.11 PID-supersession preserves this: the synthetic `SessionEnded` for each rolled-over predecessor is its *own* state+event transaction (one `write_if_state_matches` call per victim), run *after* the successor's write commits — never a third write bolted onto the successor's txn (ADR 0009 §3).
