...
**Shim logging exception:** the shim does not log to stdout/stderr on the success path or the exit-0 (daemon-answered) paths (Claude's hook environment shouldn't see daemon-internal noise), but it should log to a file (e.g. `~/.bowerbird/shim.log` with rotation). Failures must be visible somewhere; "silently exits on error" makes the shim untestable in the field. On an exit-1 failure the shim additionally emits exactly one `bowerbird: <cause>` line to stderr (with the `(see <log-path>)` pointer only when the log append succeeded) so a daemon outage renders as a named cause rather than Claude's causeless `No stderr output` (Story 5.10).
...
- **Logging on failure goes to a file**, not stdout/stderr, on the success and exit-0 (daemon-answered) paths. The shim runs inside Claude's hook environment — anything on stdout/stderr on those paths risks polluting Claude's experience. Proposed path: `~/.bowerbird/shim.log` with size-based rotation. **Scoped exception (Story 5.10):** on an exit-1 failure the shim emits exactly one `bowerbird: <cause>` line to stderr (file-log pointer appended only when the log append succeeded). Claude already surfaces a hook error on a non-zero exit, so this replaces its causeless `No stderr output` with a named cause without polluting the success path.
