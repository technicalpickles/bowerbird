...
### 6. Supersession completion is best-effort (bounded residual risk, accepted for v0.1.0)

The supersession follow-up runs *after* S′'s event has already committed, as a separate set of `write_if_state_matches` transactions (§3). It is deliberately **not** atomic with S′'s write: folding victim-ends into S′'s transaction would break the two-writes-per-txn invariant and make a single victim-write failure fail S′'s otherwise-good write. The cost is a **bounded residual window** — if the daemon crashes, or the follow-up's pool checkout / victim SELECT / clock read fails, between S′'s durable commit and a predecessor's supersession write, that predecessor stays non-`Ended`. It recovers when either (a) S′ emits its next PID-carrying event and supersession re-runs, or (b) the PID dies and the ADR 0004 probe reaps the predecessor via `pid_dead` (its `last_pid` is still `P`). A `claude` PID does not live forever, so there is no truly-indefinite leak. The contract is therefore **"retry on the next successor event, with PID-death as the eventual backstop,"** not "atomic on first observation." A durable startup/probe co-PID reconciliation pass — closing the window faster *and* sweeping predecessors stranded before this rule shipped — is deferred to follow-up bean `gt-suf7`; it edges into the `bowerbird gc` the no-list defers post-V1 (cf. the rejected upgrade-sweep alternative below). The Story 5.11 review surfaced this; the residual risk is named here rather than patched.

A related **granularity** note (Story 5.11 review pass 2): the same-state interleave guard added for review finding #3 (`WritePrecondition.expected_last_event_at_ms`, §3) pins millisecond wall-clock `last_event_at_ms` (`current_unix_millis`), which is *non-decreasing*, not strictly monotonic — two serialized writes can share a millisecond. So an equal-millisecond same-state re-emit by a victim could defeat the guard. This is **unreachable today**: supersession runs only inside the serial ingest task, and the probe only ends *dead*-PID sessions, so no other writer can interleave a same-state write on a *live*-PID victim between the scan and the synthetic write. It becomes reachable precisely when the `gt-suf7` reconciliation pass runs on the probe timer and writes to live-PID victims concurrently with ingest — so that pass must key its guard and survivor-selection on a strictly-monotonic `events.event_id`, not `last_event_at_ms`. The millisecond guard is left in place as cheap defense-in-depth for the serial path; the event-id upgrade lands with `gt-suf7`, where the concurrency that needs it is introduced.

### 7. `/replay` does not run supersession

Supersession runs only on **live ingest**, never on `POST /replay`. Replayed events flow through the same writer as live hooks (`ingest_tx` → `projection::session::write*`), but the synthetic `SessionEnded { reason: pid_superseded }` rows a prior live run produced are **already durable in the log** being replayed — replay re-applies them faithfully (each carries `kind: SessionEnded`, which the live supersession gate already skips). Re-running supersession during replay would (a) **double-generate** — replaying S′'s forward event would synthesize a *new* `SessionEnded` for the predecessor, and then the original synthetic line would also replay, accreting duplicate lifecycle rows; and (b) **corrupt a live DB** — replaying older co-PID history into a running daemon could end the *current* live holder of a PID purely on replay arrival order. So `api/replay.rs` tags its envelopes `IngestOrigin::Replay` and the ingest writer routes them to a supersession-free entrypoint (`projection::session::write_replayed`). Replay's contract is faithful reconstruction (it already drops `event_id`/`created_at` per Story 4.1); generating *new* lifecycle events from replay arrival order is not part of it. (Story 5.11 review finding #2.)
