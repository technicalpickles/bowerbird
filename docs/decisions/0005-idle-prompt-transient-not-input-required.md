...
- **Existing rows: one-time startup repair (added during code review).** The `transition` change only affects *future* projections, and "preserve prior" deliberately keeps a stored `WaitingInput` as `WaitingInput`. So rows that the *pre-5.6* daemon wrote as `WaitingInput` from an `idle_prompt` would stay stuck after deploy until the session received a non-idle hook. To meet the "drain the live-idle wall" success criterion, the daemon runs a one-time, idempotent repair at startup (`projection::session::repair_idle_prompt_waiting_input`): for each existing `WaitingInput` row it re-folds the post-5.6 `transition` over the session's event log and rewrites only when the result changes. A genuine block (`permission_prompt`/`elicitation_dialog`, even with a trailing idle nudge) recomputes back to `WaitingInput` and is left untouched. **This is a data reprojection, not a schema migration** — it adds no SQLite migration, no column, and no protocol change, so the story's "no migration / no new field" constraint still holds. It reuses the same replay path as `rebuild_missing_projections` (shared `recompute_state_from_log`), so the two cannot drift.
