...
- **launchd owns the lifecycle; install/start/uninstall disarm competitors first.** Bootstrapping over a daemon launchd does not own would fail the singleton PID lock, and `KeepAlive={SuccessfulExit=false}` would turn that into a crash-restart loop. So macOS install, before bootstrap, `bootout`s an already-loaded agent and re-bootstraps the freshly-written plist (also picking up changed `ProgramArguments`/env on reinstall), or stops a manual/pre-5.9 PID-file daemon (failing loudly rather than looping). `bowerbird start` drives launchd when the agent is registered rather than spawning a competing `setsid` daemon, and `bowerbird uninstall` adds a PID-file stop fallback so a manual daemon does not survive uninstall.
